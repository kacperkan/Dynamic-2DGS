...
def load_K_Rt_from_P(filename, P=None):
    if P is None:
        ...
        if len(lines) == 4:
            lines = lines[1:]
        lines = [
            [x[0], x[1], x[2], x[3]] for x in (x.split(" ") for x in lines)
        ]
...
def readColmapCameras(
    cam_extrinsics, cam_intrinsics, images_folder, msk_folder=None
):
    cam_infos = []
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if msk_folder is not None and image.size[-1] == 3:
            ...
            image = np.concatenate(
                [np.asarray(image), np.asarray(mask)], axis=-1
            )
...
def readColmapSceneInfo(
    ...
    eval,
    llffhold=16,
    apply_cam_norm=False,
    recenter_by_pcl=False,
):
    sparse_name = (
        "sparse"
    ...
    try:
        cameras_extrinsic_file = os.path.join(
            path, f"{sparse_name}/0", "images.bin"
        )
        cameras_intrinsic_file = os.path.join(
            path, f"{sparse_name}/0", "cameras.bin"
        )
        ...
        cameras_intrinsic_file = os.path.join(
            path, f"{sparse_name}/0", "cameras.txt"
        )
    ...
    if recenter_by_pcl:
        ply_path = os.path.join(
            path, f"{sparse_name}/0/points3d_recentered.ply"
        )
    elif apply_cam_norm:
        ply_path = os.path.join(
            path, f"{sparse_name}/0/points3d_normalized.ply"
        )
    else:
...
def readCamerasFromTransforms(
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            if frame["file_path"].endswith("jpg") or frame[
                "file_path"
            ].endswith("png"):
    ...
    if not os.path.exists(ply_path):
        if os.path.exists(os.path.join(path, "rgbd")):
            ...
            def load_depth(filepath):
                with open(filepath, "rb") as depth_fh:
                    ...
                    depth_img = np.frombuffer(
                        decompressed_bytes, dtype=np.float32
                    )
            ...
            gt_depth = torch.nn.functional.interpolate(
                gt_depth[None, None], (h, w)
            )[0, 0]
            far, near = viewpoint_camera.zfar, viewpoint_camera.znear
            u, v = torch.meshgrid(
                torch.linspace(0.5, w - 0.5, w, device=gt_depth.device) / w * 2
                - 1,
                torch.linspace(0.5, h - 0.5, h, device=gt_depth.device) / h * 2
                - 1,
                indexing="xy",
            )
...
def readNerfiesCameras(path, inter_valid=True):
    ...
    _ = [meta_json[i]["camera_id"] for i in all_img]
    ...
    if os.path.exists(f"{path}/rgb/rgba"):
        ...
        msk_path = [
            f"{path}/resized_mask/{int(1 / ratio)}x/{i}.png.png"
            for i in all_img
        ]
        msk_path = (
            msk_path
    ...
    if inter_valid:
        ...
        for idx in range(len(train_img)):
            ...
            if msk_path is not None:
                ...
                image = Image.fromarray(
                    np.concatenate([np.asarray(image), mask], axis=-1).astype(
                        "uint8"
                    )
                )
...
def readNerfiesColmapCameras(path):
    ...
    name2idx = {
        cam.image_name: idx for idx, cam in enumerate(cam_infos_unsorted)
    }
...
def readNerfiesInfo(path, eval):
    ...
    if os.path.exists(os.path.join(path, "colmap")):
        ...
        cam_infos, train_num, scene_center, scene_scale = readNerfiesCameras(
            path
        )
    ...
    if os.path.exists(os.path.join(path, "colmap")):
        ...
        if recenter_by_pcl:
            ...
            ply_path = os.path.join(
                path, f"colmap/{sparse_name}/0/points3d.ply"
            )
        ...
        adj_path = os.path.join(
            path, f"colmap/{sparse_name}/0/camera_adjustment"
        )
        if not os.path.exists(ply_path):
            print(
                "Converting point3d.bin to .ply, will happen only the first time you open the scene."
...
def readCamerasFromNpy(path, npy_file, split, hold_id, num_images):
    ...
    bottoms = (
        np.array([0, 0, 0, 1]).reshape(1, -1, 4).repeat(poses.shape[0], axis=0)
    )
    ...
    video_list = (
        i_test
        if split != "train"
        else list(set(np.arange(n_cameras)) - set(i_test))
    )
...
def readCMUInfo(path, split):
    ...
    for t in range(num_timesteps):
        for c in range(len(md["fn"][t])):
            ...
            im = np.array(
                copy.deepcopy(Image.open(f"{path}/ims/{image_name}"))
            )
            ...
            try:
                seg = np.array(
                    copy.deepcopy(
                        Image.open(
                            f"{path}/seg/{image_name.replace('.jpg', '.png')}"
                        )
                    )
                ).astype(np.float32)
            except Exception:
            ...
            image = np.asarray(
                Image.open(os.path.join(image_path, image_name))
            )
            if seg is not None:
