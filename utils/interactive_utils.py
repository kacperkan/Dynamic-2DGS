...
class DeformKeypoints:
    ...
    def add_kpts(self, keypoints_coord, keypoints_idx, expand=False):
        ...
        for idx in range(len(keypoints_idx)):
            if not self.contain_kpt(keypoints_idx[idx].item()):
                selective_keypoints_idx_list.append(
                    len(self.keypoints_idx_list)
                )
                self.keypoints_idx_list.append(keypoints_idx[idx].item())
                self.keypoints3d_list.append(
                    keypoints_coord[idx].cpu().numpy()
                )
    ...
    def get_selective_center(
        ...
        selected_points = self.get_deformed_kpt_np()[
            self.selective_keypoints_idx_list
        ]
