...
class ARAPDeformer:
    ...
    def deform(self, handle_idx, handle_pos, init_verts=None, return_R=False):
        ...
        NUM_ITER = 3
        D = torch.diag_embed(
            self.normalized_weight, dim1=1, dim2=2
        )  # [Nv, K, K]
        for _ in range(NUM_ITER):
            ...
            S = torch.bmm(
                P.permute(0, 2, 1), torch.bmm(D, P_prime)
            )  # [Nv, 3, 3]
            ...
            Rsum_batch, P_batch = (
                Rsum.view(-1, 3, 3),
                P.view(-1, 3).unsqueeze(-1),
            )

            # RHS of minimum energy equation
            b = 0.5 * (
                torch.bmm(Rsum_batch, P_batch)
                .squeeze(-1)
                .reshape(self.N, self.K, 3)
...
if __name__ == "__main__":
    ...
    trimesh.Trimesh(vertices=points_prime.cpu().numpy()).export(
        "deformation_after.ply"
    )
