...
class LapDeform(nn.Module):
    def __init__(self, init_pcl, K=4, trajectory=None, node_radius=None):
        ...
        self._weight = nn.Parameter(
            torch.log(
                nn_dist / (nn_dist.sum(dim=1, keepdim=True) + 1e-5) + 1e-5
            )
        )
        ...
        if self.mask_control_points:
            ...
            radius = (
                torch.linalg.norm(
                    self.init_pcl.max(dim=0).values
                    - self.init_pcl.min(dim=0).values
                )
                / 8
            )
            print("Set ball query radius to %f" % radius.item())
    ...
    def estimate_R(self, pcl, return_quaternion=True):
        ...
        S = torch.bmm(
            old_edges.permute(0, 2, 1), torch.bmm(D, edges)
        )  # N, 3, 3
        unchanged = torch.unique(
            torch.where((edges == old_edges).all(dim=1))[0]
        )
        S[unchanged] = 0
        U, _, W = torch.svd(S)
    ...
    def deform(self, handle_idx, handle_pos, static_idx=None):
        ...
        if static_idx is not None:
            ...
            handle_pos = torch.cat(
                [handle_pos.cuda(), static_pos.cuda()], dim=0
            )
    ...
    def deform_arap(
        self, handle_idx, handle_pos, init_verts=None, return_R=False
    ):
