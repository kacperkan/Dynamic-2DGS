...
class GUI:
    def __init__(
        ...
        self.deform = DeformModel(
            ...
            d_rot_as_res=self.dataset.d_rot_as_res
            and not self.dataset.d_rot_as_rotmat,
        ...
        deform_loaded = self.deform.load_weights(
            dataset.model_path, iteration=-1
        )
        ...
        self.iteration_node_rendering = (
            ...
            else self.opt.iterations_node_rendering
        )
    ...
    def train_step(self):
        ...
        if not self.viewpoint_stack:
            if self.opt.progressive_train and self.iteration < int(
                ...
                cameras_to_train_idx = int(
                    min(
                        (
                            (self.iteration) / self.opt.progressive_stage_steps
                            + 1
                        )
                        * self.opt.progressive_stage_ratio,
                        1.0,
                ...
                viewpoint_stack = sorted_train_cams[
                    min_idx:cameras_to_train_idx
                ]
        ...
        if self.deform.name == "mlp" or self.deform.name == "static":
            ...
            if not self.deform.deform.inited:
                print(
                    "Notice that warping nodes are initialized with Gaussians!!!"
                )
                self.deform.deform.init(
                    self.opt,
                    self.gaussians.get_xyz.detach(),
        ...
        if (
            ...
            if (
                flow.shape[0] != image.shape[1]
                or flow.shape[1] != image.shape[2]
            ):
                ...
                masks = torch.nn.functional.interpolate(
                    masks.permute([2, 0, 1])[None],
                    (image.shape[1], image.shape[2]),
                )[0].permute(1, 2, 0)
            fid1 = viewpoint_cam.fid
            cam2_id = (
                os.path.basename(flow_id2_dir).split("_")[-1].split(".")[0]
            )
            ...
            if (
                cam2_id in self.img2cam
            ):  # Only considering the case with existing files
                ...
                coor1to2_flow = (
                    flow
                    / torch.tensor(
                        flow.shape[:2][::-1], dtype=torch.float32
                    ).cuda()
                    * 2
                )
        ...
        with torch.no_grad():
            ...
            if self.iteration < self.opt.densify_until_iter:
                ...
                if (
                    ...
                    and self.iteration % self.opt.node_densification_interval
                    == 0
                    ...
                    self.deform.densify(
                        ...
                        x_grad=self.gaussians.xyz_gradient_accum
                        / self.gaussians.denom,
                        feature=self.gaussians.feature,
                        force_dp=(
                            self.iteration
                            == self.opt.node_force_densify_prune_step
                        ),
                    )
    ...
    def train_node_rendering_step(self):
        ...
        if not self.viewpoint_stack:
            if (
                ...
                if self.iteration_node_rendering < self.opt.node_warm_up:
                    ...
                    if len(out_domain_idx) >= interval_len:
                        out_domain_len = min(
                            interval_len * 5, len(out_domain_idx)
                        )
                        out_domain_idx = np.random.choice(
        ...
        d_values = self.deform.deform.query_network(
            x=self.deform.deform.as_gaussians.get_xyz.detach(),
            t=time_input + ast_noise,
        )
        ...
        with torch.no_grad():
            if (
                ...
                if (
                    self.iteration_node_rendering
                    % self.opt.densification_interval
                    == 0
                    or self.iteration_node_rendering
                    == self.opt.node_warm_up - 1
                ):
                    size_threshold = (
                        20
                ...
                if strategy == "direct":
                    original_gaussians: GaussianModel = (
                        self.deform.deform.as_gaussians
                    )
                    self.deform.deform.init(
                        opt=self.opt,
                    ...
                    gaussians._scaling = torch.nn.Parameter(
                        original_gaussians._scaling
                    )
                    gaussians._opacity = torch.nn.Parameter(
                        original_gaussians._opacity
                    )
                    ...
                    for i in range(time_num):
                        time_input = t_samp[i : i + 1, None].expand_as(
                            x[..., :1]
                        )
                        trans_samp.append(
                            self.deform.deform.query_network(
                                x=x, t=time_input
                            )["d_xyz"]
                    ...
                    dynamic_mask = (
                        self.deform.deform.as_gaussians.motion_mask[..., 0]
                        > 0.5
                    )
                ...
                self.deform.deform.as_gaussians.optimizer.zero_grad(
                    set_to_none=True
                )
                self.deform.optimizer.zero_grad()
            ...
            if (
                not self.iteration_node_rendering
                == self.opt.iterations_node_sampling
