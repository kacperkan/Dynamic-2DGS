...
def estimate_rotation(
    source, target, ii, jj, nn, K=10, weight=None, sample_idx=None
):
...
def cal_arap_error(
    nodes_sequence, ii, jj, nn, K=10, weight=None, sample_num=512
):
    ...
    if Nv > sample_num:
        sample_idx = (
            torch.from_numpy(np.random.choice(Nv, sample_num)).long().cuda()
        )
    else:
        source_edge_mat = source_edge_mat[sample_idx]
    ...
    for idx in range(1, Nt):
        ...
        ).permute(0, 2, 1)  # [Nv, K, 3]
        stretch_vec = target_edge_mat - rot_rigid  # stretch vector
        stretch_norm = (
            torch.norm(stretch_vec, dim=2) ** 2
        )  # norm over (x,y,z) space
...
def arap_deformation_loss(
    ...
    radius = (
        torch.linalg.norm(
            init_pcl.max(dim=0).values - init_pcl.min(dim=0).values
        )
        / 8
    )
    ...
    P = produce_edge_matrix_nfmt(
        init_pcl, (N, K, 3), ii, jj, nn, device=device
    )
    P_prime = produce_edge_matrix_nfmt(
        tar_pcl, (N, K, 3), ii, jj, nn, device=device
    )
