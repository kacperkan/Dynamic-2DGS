    ...
    loss = torch.where(
        ...
        torch.where(
            ...
            torch.where(
                ...
                loss_two,
                torch.where(
                    alpha == float("inf"), loss_posinf, loss_otherwise
                ),
            ),
        ),
    )
...
class ProgressiveBandFrequency(nn.Module):
    def __init__(
        self, in_channels: int, n_frequencies=12, no_masking_step=5000
    ):
        super().__init__()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.progressive_brand_time:
            ...
            self.embed_time_fn, time_input_ch = get_embedder(
                self.t_multires, 1
            )
                ...
                for layer in self.gaussian_color:
                    if hasattr(layer, "weight"):
                        ...
                        if layer.bias is not None:
                            nn.init.zeros_(layer.bias)
                nn.init.normal_(
                    self.gaussian_color[-1].weight, mean=0, std=1e-5
                )
            ...
            if self.resnet_color:
                if self.color_wrt_dir:
                    if "camera_center" in kwargs:
                        dir_pp = x - kwargs["camera_center"].repeat(
                            x.shape[0], 1
                        )
                        ...
                        return_dict["d_color"] = self.gaussian_color(
                            torch.cat(
                                [self.embed_fn(dir_pp_normalized), h], dim=-1
                            )
                        )
                    else:
...
class ProgressiveBandHashGridCosine(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "current_step", torch.tensor(0, dtype=torch.int32)
        )
    ...
    def update_step(self, global_step):
        ...
        if self.n_masking_step <= 0 or global_step is None or global_step < 0:
            ...
            self.current_step.data = global_step * torch.ones_like(
                self.current_step
            )
            ...
            self.mask[start_idx:] = (
                ...
                - torch.cos(
                    ...
                    * (
                        ratio * band_len
                        - torch.arange(
                            0, band_len, device=torch.device("cuda:0")
                        )
                    ).clamp(0, 1)
                )
            ) / 2.0
...
class MLP(nn.Module):
    def __init__(
        ...
        for i in range(self.n_hidden_layers - 1):
            ...
            self.layers.append(self.make_activation())
        self.layers.append(
            self.make_linear(self.n_neurons, dim_out, bias=True)
        )
...
class HashDeformNetwork(nn.Module):
    def __init__(
        ...
        if not hash_time:
            self.time_embed_func, self.time_embed_dim = get_embedder(
                t_multires, 1
            )
        self.mlp = mlp_type(
...
class ControlNodeWarp(nn.Module):
        ...
        if not self.skinning:
            ...
            if self.with_node_weight:
                self._node_weight = nn.Parameter(
                    ...
                    requires_grad=with_node_weight,
                )
            ...
            self.nodes.data = nn.Parameter(
                torch.cat(
                    [
                        ...
                        1e-2
                        * torch.ones([self.node_num, self.hyper_dim])
                        .float()
                        .cuda(),
                    ],
                    dim=-1,
        ...
        if self.skinning:
            if "feature" in kwargs:
                ...
                initial_weights = -torch.log(
                    (init_pcl[:, None] - self.nodes[None, ..., :3])
                    .square()
                    .sum(dim=-1)
                    / radius**2
                )
            ...
            if keep_all or self.node_num > init_pcl.shape[0]:
                self._node_radius = nn.Parameter(
                    ...
                    * torch.ones([self.node_num])
                    .float()
                    .to(scene_range.device)
                )
                self._node_weight = nn.Parameter(
    ...
    def cal_nn_weight_floyd(
        ...
        if not hasattr(self, f"{cache_name}_nn_dist") or (
            ...
            node_dist_mat = self.geodesic_distance_floyd(
                cur_node=cur_node, K=GraphK
            )
    ...
    def node_deform(self, t, detach_node=True, **kwargs):
        ...
        values = {
            key: values[key].view(*tshape[:-1], values[key].shape[-1])
    ...
    def get_trajectory(self, t_samp_num=8):
        ...
        t_samp = t_samp[None, :, None].expand(
            self.node_num, t_samp_num, 1
        )  # M, T, 1
        node_deform = self.node_deform(t=t_samp)
        trajectory = (
            self.nodes[:, None, :3].detach() + node_deform["d_xyz"]
        )  # M, T, 3
        for key in node_deform:
            node_deform[key] = (
                node_deform[key][:, 0]
                if node_deform[key] is not None
                else None
            )
    ...
    def arap_loss_with_rot(self, t_samp_num=8):
        ...
        trajectory_rot = (
            node_deform["d_rotation"] if not self.d_rot_as_res else None
        )
    ...
    def p2dR(self, p, p0=None, K=8, as_quat=True, mode="trajectory", t0=None):
        ...
        edges, edges_t = (
            edges / (edges.norm(dim=-1, keepdim=True) + 1e-5),
            edges_t / (edges_t.norm(dim=-1, keepdim=True) + 1e-5),
        )
        W = torch.zeros(
    ...
    def elastic_loss(self, t=None, delta_t=0.005, K=2, t_samp_num=8):
        ...
        nn_weight, _, nn_idx = self.cal_nn_weight(
            x=self.nodes[..., :3].detach(),
            feature=self.nodes[..., 3:],
            K=K + 1,
        )
    ...
    def acc_loss(self, t=None, delta_t=0.005):
        ...
        acc = (nodes_t[:, 0] + nodes_t[:, 2] - 2 * nodes_t[:, 1]).norm(
            dim=-1
        )  # M
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not self.d_rot_as_res:
            ...
            # Scale residual
            scale = (node_scale[nn_idx] * nn_weight[..., None]).sum(
                dim=1
            ) * motion_mask
            return_dict = {
                "d_xyz": translate,
    ...
    def as_gaussians(self):
        if not hasattr(self, "gs") or self.gs is None:
            ...
            self.gs.create_from_pcd(
                pcd=pcd, spatial_lr_scale=0.0, print_info=False
            )
            self.gs._scaling.data = torch.log(
                1e-2 * torch.ones_like(self.gs._scaling)
            )
    ...
    def init_gaussians(self, init_pcl, with_motion_mask):
        if not hasattr(self, "gs") or self.gs is None:
            ...
            self.gs = StandardGaussianModel(
                sh_degree=0,
                all_the_same=True,
                with_motion_mask=with_motion_mask,
            )  # blender datas are all dynamic
            self.gs.create_from_pcd(
                pcd=pcd, spatial_lr_scale=0.0, print_info=False
            )
    ...
    @torch.no_grad()
    def cal_node_importance(
        self, x: torch.Tensor, K=None, weights=None, feature=None
    ):
    ...
    def densify(
        ...
        node_avg_xgradnorm, node_avg_x, node_edge_count = (
            self.cal_node_importance(
        ...
        if selected_pts_mask.sum() > 0:
            ...
            self.nodes_color_visualization = torch.cat(
                [
                    self.nodes_color_visualization,
                    torch.ones_like(new_nodes[..., :3]),
                ],
                dim=0,
            )
            self.nodes_color_visualization[-new_nodes.shape[0] :, 1:] = (
                0  # Set as red
            )
        ...
        if pruned_pts_mask.shape[0] < self.nodes.shape[0]:
            pruned_pts_mask = torch.cat(
                [
                    pruned_pts_mask,
                    torch.zeros(
                        [self.nodes.shape[0] - pruned_pts_mask.shape[0]]
                    )
            ...
            for group in optimizer.param_groups:
                ...
                for i in param_idx:
                    ...
                    if stored_state is not None:
                        ...
                        stored_state["exp_avg_sq"] = stored_state[
                            "exp_avg_sq"
                        ][pruned_pts_mask]
                        del optimizer.state[group["params"][i]]
                        group["params"][i] = nn.Parameter(
                        ...
                        group["params"][i] = nn.Parameter(
                            group["params"][i][pruned_pts_mask].requires_grad_(
                                True
                            )
                        )
                        optimizable_tensors[param_list[i]] = group["params"][i]
